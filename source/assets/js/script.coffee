...
app.controller 'TimerController', [ '$scope', '$interval', 'Util', ($scope, $interval, Util) ->
  release = new Date(2015, 6, 12, 12)
  $interval ->
    now = new Date
    diff = Math.floor (release.getTime() - now.getTime()) / 1000
    $scope.orderDate =  Util.dhms diff
  , 1000
]
